...
var add_checkbox_g_traymenu_item, add_checkbox_menu_item, add_g_traymenu_item, add_g_traymenu_seperator, add_menu_item, add_menu_seperator, add_output, cb_server_url, change_workingstate, check_all_progress, check_result, child_process, cryptobox_ctrl, cryptobox_locked_status_change, fs, g_output, g_second_counter, g_tray, g_winmain, get_all_smemory, get_motivation, get_option, get_sync_state, get_user_var, gui, on_exit, path, print, progress_checker, run_command, second_interval, set_data_user_config, set_menus_and_g_tray_icon, set_output_buffers, set_user_var_scope, spawn, start_after_second, start_process, start_watch, store_user_var, update_output, update_sync_state, warning, watch,
  __slice = [].slice,
  _this = this;
...
start_watch = function() {
  var file_watch_started, watch_path;
  if (!file_watch_started) {
    ...
    if (fs.existsSync(watch_path)) {
      ...
      return watch.watchTree(watch_path, function(f, curr, prev) {
        if (!String(f).contains("memory.pickle")) {
          ...
          if (prev === null) {
            return file_watch_started = false;
...
check_result = function(name) {
  ...
  if (result_cnt > 100) {
    return print("cryptobox.cf:170", "too many result checks", name, result_cnt);
  } else {
...
get_motivation = function(scope) {
  if (scope.motivation == null) {
    return run_command("get_motivation").then(function(motivation) {
      ...
      return print("cryptobox.cf:206", error);
    });
  } else {
...
get_sync_state = function(q, scope) {
  var option, p;
  p = q.defer();
  option = {
  ...
  run_command("run_cb_command", option).then(function() {
    return p.resolve();
  }, function(err) {
    return p.reject(err);
  });
...
set_menus_and_g_tray_icon = function(scope) {
  var actions, g_trayactions, menubar;
...
start_after_second = function() {
  get_motivation();
  get_sync_state();
  return setInterval(second_interval, 1000);
};

progress_checker = function(fname, scope, utils) {
  var data, fprogress;
...
check_all_progress = function(scope, utils) {
  progress_checker("progress", scope, utils);
  return progress_checker("item_progress", scope, utils);
};
...
cryptobox_ctrl = function($scope, $q, memory, utils) {
  ...
  $scope.form_change = function() {
    ...
    return $q.all([p_cb_folder, p_cb_username, p_cb_password, p_cb_name, p_cb_server, p_show_settings, p_show_debug]).then(function() {
      ...
      return warning("cryptobox.cf:588", err);
    });
  };
  ...
  $scope.sync_btn = function() {
    ...
    option.clear = "0";
    option.sync = "0";
    return run_command("run_cb_command", option);
  };
  ...
  start_process_once();
  get_motivation();
  return set_menus_and_g_tray_icon();
};
