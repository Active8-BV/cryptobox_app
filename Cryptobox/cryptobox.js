...
var add_checkbox_g_traymenu_item, add_checkbox_menu_item, add_g_traymenu_item, add_g_traymenu_seperator, add_menu_item, add_menu_seperator, add_output, cb_server_url, change_workingstate, check_all_progress, child_process, cryptobox_ctrl, cryptobox_locked_status_change, fs, g_menu, g_menuactions, g_output, g_second_counter, g_tray, g_trayactions, g_winmain, get_all_smemory, get_motivation, get_option, get_sync_state, get_user_var, gui, on_exit, option_to_string, path, print, progress_checker, run_cba_main, second_interval, set_data_user_config, set_menus_and_g_tray_icon, set_output_buffers, set_user_var_scope, sleep, spawn, start_watch, store_user_var, update_output, update_sync_state, warning, watch,
  __slice = [].slice;
...
option_to_string = function(option) {
  var cmd_str;
  cmd_str = "";
...
run_cba_main = function(options) {
  var cba_main, cmd_to_run, params;
  ...
  return cba_main.wait();
};

get_motivation = function(scope) {
  var get_motivation_scope, options,
    _this = this;
  if (scope.motivation == null) {
    return options = scope.motivation = run_cba_main("get_motivation", "", scope);
  } else {
...
cryptobox_ctrl = function($scope, memory, utils) {
  var option, second_interval_scoped, set_data_user_config_once;
  ...
  return print("cryptobox.cf:566", option_to_string(option));
};
