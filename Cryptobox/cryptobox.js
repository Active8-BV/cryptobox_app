...
parse_json = function(data, returnlist, debug) {
...
on_exit = function() {
  if (g_cba_main != null) {
    if (typeof g_cba_main.kill === "function") {
      g_cba_main.kill();
    }
  }
  return gui.App.quit();
...
set_sync_check_on_scope = function(scope, sync_results) {
  var human_readable_size, human_readable_size2;
  ...
  if (sync_results.file_del_server != null) {
    return scope.file_del_server = sync_results.file_del_server;
  }
};

update_sync_state = function(scope) {
  var give_list, option, result_sync_state;
  option = {
      ...
      process_sync_result = function(sync_results) {
        ...
        if (sync_results.instruction != null) {
          ...
          try {
            ...
            return print("cryptobox.cf:458", sync_results);
          }
        }
      };
  ...
  give_list = true;
  return run_cba_main("update_sync_state", option, result_sync_state, null, give_list);
};
...
cryptobox_locked_status_change = function(locked, scope) {
  ...
  if (scope.cryptobox_locked) {
    ...
    if (g_decrypt_g_tray_item != null) {
      return g_decrypt_g_tray_item.enabled = true;
    }
  } else {
    ...
    if (g_decrypt_g_tray_item != null) {
      return g_decrypt_g_tray_item.enabled = false;
    }
  }
};
...
set_motivation = function($scope) {
  ...
  motivation_cb = function(result, output) {
    if (result) {
      return $scope.motivation = output != null ? output.motivation.replace("\n", "<br/>") : void 0;
    }
  };
...
g_progress_callback = function(scope, output) {
  ...
  try {
    ...
    err = _error;
    return print("cryptobox.cf:602", "g_progress_callback", err);
  }
};
...
delete_blobs = function(scope) {
  ...
  cb_delete_blobs = function(result, output) {
    return print("cryptobox.cf:631", "blobs deleted", result, output);
  };
...
check_for_new_release = function(scope) {
  ...
  return run_cba_main("check_new_release", option, cb_check_new_release);
};

angular.module("cryptoboxApp", ["cryptoboxApp.base", "angularFileUpload"]);

cryptobox_ctrl = function($scope, memory, utils, $q) {
  var check_feedback_progress_callback, digester, do_sync, g_window_width, once_motivation, progress_callback, ten_second_interval, two_second_interval;
  ...
  $scope.download_new_release = function() {
    ...
    return run_cba_main("download_new_release", option, null, progress_callback);
  };
  ...
  $scope.reset_cache = function() {
    ...
    option.encrypt = true;
    option.clear = true;
    return run_cba_main("reset_cache", option, null, progress_callback);
  };
  $scope.form_save = function() {
    ...
    get_user_var("cb_password", $q).then(function(oldpassword) {
      ...
      return print("cryptobox.cf:743", "error setting password", err);
    });
    $scope.form_changed = false;
  ...
  do_sync = function() {
    ...
    return run_cba_main("sync server", option, sync_cb, check_feedback_progress_callback);
  };
  ...
  $scope.open_folder = function() {
    ...
    return run_cba_main("open_folder", option);
  };
  $scope.open_website = function() {
    var option;
    option = get_option($scope);
    option.acommand = "open_website";
    return run_cba_main("open_website", option);
  };
  g_window_width = 300;
  $scope.set_window_size_settings = function() {
    var make_bigger, make_smaller;
    if ($scope.show_settings) {
      make_bigger = function() {
        ...
        if (g_window_width < 800) {
          return setTimeout(make_bigger, 1);
        }
      };
      return make_bigger();
    } else {
      make_smaller = function() {
        ...
        g_winmain.resizeTo(g_window_width, 700);
        if (g_window_width > 500) {
          return setTimeout(make_smaller, 1);
        }
      };
      return make_smaller();
    }
  ...
  $scope.toggle_debug = function() {
    ...
    return print("cryptobox.cf:856", $scope.show_debug);
  };
  ...
  digester = function() {
    ...
    if (g_info_message != null) {
      ...
      return setTimeout(clear_info, 5000);
    }
  };
