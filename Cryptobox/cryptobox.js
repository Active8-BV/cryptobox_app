...
run_cba_main = function(name, options, cb, cb_stdout) {
  var cba_main, cmd_to_run, error, execution_done, output, params, stdout_data;
  if (!exist(cb)) {
...
update_sync_state = function(scope) {
  ...
  result_sync_state = function(result, sync_results) {
    var ex;
    if (result) {
...
set_motivation = function($scope) {
  ...
  motivation_cb = function(result, output) {
    if (result) {
      return $scope.motivation = output.motivation.replace("\n", "<br/>");
    }
  };
...
g_progress_callback = function(scope, output) {
  ...
  try {
    ...
    return print("cryptobox.cf:547", err);
  }
};
...
cryptobox_ctrl = function($scope, memory, utils, $q) {
  ...
  $scope.reset_cache = function() {
    ...
    clear_cb = function(result, output) {
      return print("cryptobox.cf:713", result, output);
    };
