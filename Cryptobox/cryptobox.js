...
var add_checkbox_g_traymenu_item, add_checkbox_menu_item, add_g_traymenu_item, add_g_traymenu_seperator, add_menu_item, add_menu_seperator, add_output, already_running, cb_server_url, child_process, cryptobox_ctrl, cryptobox_locked_status_change, fs, g_cba_main, g_encrypt_g_tray_item, g_error_message, g_menu, g_menuactions, g_output, g_progress_callback, g_second_counter, g_tray, g_trayactions, g_winmain, get_option, get_user_var, gui, on_exit, option_to_array, parse_json, path, print, reset_bars, reset_bars_timer, run_cba_main, set_data_user_config, set_menus_and_g_tray_icon, set_motivation, set_sync_check_on_scope, set_user_var_scope, start_watch, store_user_var, update_sync_state, warning, watch,
  __slice = [].slice;
...
add_output = function(msgs) {
  var add_msg;
  if ((msgs != null ? typeof msgs.indexOf === "function" ? msgs.indexOf(".cf") : void 0 : void 0) >= 0) {
    msgs = "> " + msgs;
  }
...
g_progress_callback = function(scope, output) {
  var clear_info, err, stored_output;
  ...
  try {
    ...
    return print("cryptobox.cf:553", err);
  }
};
...
cryptobox_ctrl = function($scope, memory, utils, $q) {
  ...
  check_feedback_progress_callback = function(output) {
    var progress_output, sync_results;
    progress_output = output;
    g_progress_callback($scope, progress_output);
    sync_results = parse_json(output);
    return set_sync_check_on_scope($scope, sync_results);
  };
  do_sync = function() {
    var option, sync_cb;
  ...
  $scope.open_folder = function() {
    ...
    open_cb = function(result, output) {
      return print("cryptobox.cf:712", result, output);
    };
  ...
  $scope.reset_cache = function() {
    ...
    clear_cb = function(result, output) {
      return print("cryptobox.cf:721", result, output);
    };
