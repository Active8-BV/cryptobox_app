...
cryptobox_ctrl = ($scope, $q, memory, utils) ->
    print "cryptobox.cf:39", "cryptobox_ctrl"
    ...
    $scope.on_exit = =>
        ...
        #print "cryptobox.cf:54", "cryptobox app on_exit"
        ...
        client.methodCall "force_stop",[], (e,v) ->
            ...
            utils.set_time_out("cryptobox.cf:65", force_kill, 100)
    ...
    get_motivation = ->
        if not utils.exist($scope.motivation)
            ...
            client.methodCall "get_motivation", [], (error, value) ->
                ...
                if not utils.exist($scope.motivation)
                    utils.set_time_out("cryptobox.cf:154", get_motivation, 100)
    ...
    start_process = =>
        ...
        client.methodCall "force_stop",[], (e,v) ->
            if utils.exist(v)
                print "cryptobox.cf:174", "killed existing deamon"
            else
                print "cryptobox.cf:176", "starting deamon"
    ...
    reset_progress = ->
        ...
        client.methodCall "reset_progress",[], (e,v) ->
            if utils.exist(e)
                warning "cryptobox.cf:203", e

    reset_item_progress = ->
        client = get_rpc_client()
        client.methodCall "reset_item_progress",[], (e,v) ->
            if utils.exist(e)
                warning "cryptobox.cf:209", e
    ...
    get_progress = (progress, progress_item) =>
        ...
        if progress_bar >= 100
            ...
            utils.set_time_out("cryptobox.cf:248", reset_progress_bar, 500)
        ...
        if progress_bar_item >= 100
            ...
            utils.set_time_out("cryptobox.cf:256", reset_progress_bar_item, 500)
        utils.force_digest($scope)
    ...
    try_get_sync_state = =>
        ...
        get_sync_state().then(
            (r) ->
                print "cryptobox.cf:344", "sync state retrieved"

            (e) ->
                warning "cryptobox.cf:347", e
        )

    start_watch = ->
        if $scope.got_folder_text and $scope.got_cb_name
            if fs.exists(path.join($scope.cb_folder_text, $scope.cb_name))
    ...
    start_after_second = =>
        ...
        try_get_sync_state()
        utils.set_interval("cryptobox.cf:681", second_interval, 1000, "second_interval")

    utils.set_time_out("cryptobox.cf:683", start_after_second, 1000)
