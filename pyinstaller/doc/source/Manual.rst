...
For example ``hook-msgpack.py`` is a hook file telling
about hidden imports used by the module ``msgpack``.
When your script has ``import msgpack``
the Analysis will note it and check for a hook file ``hook-msgpack.py``.

Typically a hook module has only one line; in ``hook-msgpack.py`` it is
